...
namespace WAT.Models
{
    public class WaferTrace
    {
        ...
        {
            var sql = "select Priority from [WAT].[dbo].[WaferTrace] where WaferNum = @WaferNum and TestStuatus=''";
            var dict = new Dictionary<string, string>();
            dict.Add("@WaferNum", wafer);
        ...
        public static void SendJOEmail(WaferTrace wtrace,Controller ctrl)
        {
            try
            {
                ...
                line.Add("<a href='http://wux-engsys01.chn.ii-vi.net:9090/Main/StartWATJO?wf="+WebUtility.UrlEncode(wtrace.WaferNum)+"' target='_blank'>DONE</a>");
