...
{
    public class WXWATTTFuse
    {
        public static List<WXWATTTFuse> GetTTFuseData(List<WXSpecBinPassFail> ttpspec, List<WXWATTTFfit> fitsrcdata)
        {
            var ret = new List<WXWATTTFuse>();
            foreach (var spec in ttpspec)
            {
                foreach (var fitem in fitsrcdata)
                {
                    if (string.Compare(spec.Bin_PN, fitem.Bin_PN, true) == 0)
                    {
                        ...
                        if (ref4 != 0)
                        {
                            var z = WXWATTTFSorted.GetNormalSinV(tempvm.TTF_percent / ref4);
