...
{
    public class WXWATTTFSorted:WXWATTTF
    {
        public static List<WXWATTTFSorted> GetSortedTTFData(List<WXWATTTF> srcdata)
        {
            var ttfdict = new Dictionary<string, List<WXWATTTF>>();
            foreach (var item in srcdata)
            {
                ...
                {
                    var templist = new List<WXWATTTF>();
                    templist.Add(item);
                    ttfdict.Add(item.Bin_PN, templist);
                }
            }//end foreach

            var ret = new List<WXWATTTFSorted>();
            foreach (var kv in ttfdict)
            {
                var pnttflist = kv.Value.ToList();
                pnttflist.Sort(delegate (WXWATTTF obj1, WXWATTTF obj2)
                {
                ...
                foreach (var item in pnttflist)
                {
                    ...
                    ret.Add(new WXWATTTFSorted(item, cumft, log0, log1, z));
        ...
        private WXWATTTFSorted(WXWATTTF data, double cumft, string log0, string log1, double zz)
        {
